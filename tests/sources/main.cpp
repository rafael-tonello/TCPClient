...
int main3(int argc, char* argv[]){
    ...
    cli.addConEventListener([](TCPClientLib::TCPClient *cli, TCPClientLib::CONN_EVENT evt)
    {
        if (evt == TCPClientLib::CONN_EVENT::CONNECTED)
    ...
    cli.connectToServer("127.0.0.1", 8085).get();

    cli.sendString("Mas olha aí galera!!");
